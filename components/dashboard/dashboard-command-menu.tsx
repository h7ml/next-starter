...
import { FileText, BarChart3, Settings, PenLine, Search } from "lucide-react"
import {
  CommandDialog,
  CommandEmpty,
  CommandGroup,
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        ...
        <CommandList>
          ...
          <CommandGroup heading={dict.dashboard.quickNav}>
            <CommandItem onSelect={() => runCommand(() => router.push(`/${locale}/dashboard`))}>
              <BarChart3 className="mr-2 h-4 w-4" />
              <span>{dict.dashboard.overview}</span>
            </CommandItem>
            <CommandItem
              onSelect={() => runCommand(() => router.push(`/${locale}/dashboard/posts`))}
            >
              <FileText className="mr-2 h-4 w-4" />
              <span>{dict.dashboard.myPosts}</span>
            </CommandItem>
            <CommandItem
              onSelect={() => runCommand(() => router.push(`/${locale}/dashboard/posts/new`))}
            >
              <PenLine className="mr-2 h-4 w-4" />
              <span>{dict.dashboard.newPost}</span>
            </CommandItem>
            <CommandItem
              onSelect={() => runCommand(() => router.push(`/${locale}/dashboard/analytics`))}
            >
              <BarChart3 className="mr-2 h-4 w-4" />
              <span>{dict.dashboard.analytics}</span>
            </CommandItem>
            <CommandItem
              onSelect={() => runCommand(() => router.push(`/${locale}/dashboard/settings`))}
            >
